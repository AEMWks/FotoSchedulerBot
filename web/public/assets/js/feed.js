...
class PhotoFeed {
    ...
    async loadAllPhotos() {
        ...
        try {
            ...
            console.log(`Cargadas ${totalCount} fotos de ${Object.keys(allPhotos).length} días`);
    ...
    generateMockPhotos(dateStr) {
        ...
        const now = Date.now();
        const daysSinceDate = Math.floor((now - new Date(dateStr).getTime()) / (1000 * 60 * 60 * 24));
            ...
            html += `
                <div class="diary-entry">
                    ...
                    <div class="media-grid">
                        ${photos.map((photo, index) => this.createMediaItem(photo, date, index)).join('')}
                    </div>
                </div>
            `;
        }
        ...
        try {
            ...
            for (const date of this.allPhotoDates) {
                ...
                if (currentPhotos.length > 0) {
                    ...
                    if (currentPhotos.length !== existingPhotos.length ||
    ...
    previousMedia() {
        ...
        this.lightbox.currentIndex = (this.lightbox.currentIndex - 1 + this.lightbox.currentMedia.length) % this.lightbox.currentMedia.length;
    ...
    nextMedia() {
        ...
        this.lightbox.currentIndex = (this.lightbox.currentIndex + 1) % this.lightbox.currentMedia.length;
        this.updateLightboxContent();
    }
    ...
    updateSlideshowContent() {
        ...
        progress.textContent = `${this.slideshow.currentIndex + 1} / ${this.slideshow.allMedia.length}`;
    }
    ...
    nextSlide() {
        this.slideshow.currentIndex = (this.slideshow.currentIndex + 1) % this.slideshow.allMedia.length;
        this.updateSlideshowContent();
    }
    ...
    // Utility functions
    updateStats() {
        this.totalPhotos = Object.values(this.loadedPhotos).reduce((sum, photos) => sum + photos.length, 0);
