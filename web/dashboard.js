...
class PhotoDashboard {
    ...
    async loadAvailableDates() {
        try {
            const response = await fetch(`${this.API_BASE}/dates?format=simple&limit=365`);
            const data = await response.json();

            if (data.success) {
                this.allPhotoDates = data.data.dates.map(d => d.date || d);
            } else {
            ...
            await this.loadAvailableDatesLegacy();
        }
    }
    ...
    async performAdvancedSearch() {
        ...
        try {
            const response = await fetch(`${this.API_BASE}/search?${searchParams.toString()}`);
            const data = await response.json();
    ...
    async exportContent(type, options = {}) {
        ...
        switch (type) {
            ...
            case 'week':
                exportParams.append('date', options.date || this.formatDate(new Date()));
                break;

            case 'month':
                exportParams.append('month', options.month || this.formatMonth(new Date()));
                break;
        ...
        try {
            ...
            const response = await fetch(`${this.API_BASE}/export?${exportParams.toString()}`);
    ...
    async getRandomMemory(count = 1, excludeRecentDays = 0) {
        try {
            ...
            const response = await fetch(`${this.API_BASE}/random?${params.toString()}`);
            const data = await response.json();
    ...
    async loadCalendarData(year, month) {
        try {
            const response = await fetch(`${this.API_BASE}/calendar/${year}/${month}`);
            const data = await response.json();
...
class PhotoFeed {
    ...
    async loadFeedPage(page = 1, limit = 10) {
        ...
        try {
            ...
            const response = await fetch(`${this.API_BASE}/feed?${params.toString()}`);
            const data = await response.json();
            ...
            if (data.success) {
                ...
                console.log(`Página ${page} cargada: ${data.data.feed.length} entradas`);
            } else {
    ...
    async refreshFeed() {
        try {
            ...
            const response = await fetch(`${this.API_BASE}/feed?page=1&limit=5&include_activity=true&activity_days=1`);
            const data = await response.json();
...
window.dashboardAPI = {
    ...
    exportWeek: () => dashboard.exportContent('week', { date: dashboard.formatDate(new Date()) }),
    exportMonth: () => dashboard.exportContent('month', { month: dashboard.formatMonth(new Date()) }),
